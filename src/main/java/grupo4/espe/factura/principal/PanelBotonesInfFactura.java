...
public class PanelBotonesInfFactura extends JPanel implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (comando.equals(GENERAR_FACTURA)) {
			...
			/*
			 * //String fechaDeFactura = datos.darFechaFactura(); String
			 * numeroDeFacturaString = datos.darNumFactura(); String cedula =
			 * datos.darCedula(); String nombreCliente =
			 * datos.darNombre().toString().trim(); String telefono =
			 * datos.darTelefono().toString().trim(); String correo =
			 * datos.darCorreo().toString().trim(); String direccion =
			 * datos.darDireccion().toString().trim();
			 * 
			 * Factura factura = new Factura( numeroDeFacturaString, cedula, nombreCliente,
			 ...
			 * 
			 * try { DB db = mongo.conectarMongoDB(); DBCollection coleccion =
			 * (DBCollection) db.getCollection("Facturas"); for(Factura fac: facturas) {
			 * 
